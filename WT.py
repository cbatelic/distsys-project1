...
async def get_links(request):
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("http://localhost:5003/gatherdata", json = res) as res:
                result = await res.json()
        return web.json_response({"status":"ok","passedFirstLetters": result.get("recivedData"),"recivedDataFromMicroservisM0":data}, status=200)
